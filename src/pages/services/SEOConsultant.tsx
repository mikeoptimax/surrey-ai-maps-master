import { Button } from "@/components/ui/button";
import { Link } from "react-router-dom";

const SEOConsultant = () => {
  return (
    <div className="min-h-screen">
      <div className="container mx-auto px-4 py-24">
        <div className="text-center mb-16">
          <Link to="/services" className="text-orange hover:underline mb-4 inline-block">← Back to Services</Link>
          <h1 className="headline-xl text-navy mb-8">
            EXPERT MARKETING<br />
            CONSULTATION
          </h1>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Expert marketing consultation tailored to Surrey businesses. Custom strategies 
            that align with your industry, competition, and growth goals.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            <div className="brutal-card mb-8">
              <h2 className="headline-lg text-navy mb-6">What's Included</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ul className="space-y-3">
                  <li>• SEO Strategy Development</li>
                  <li>• Local Marketing Planning</li>
                  <li>• Digital Marketing Consultation</li>
                </ul>
                <ul className="space-y-3">
                  <li>• Industry-Specific Marketing</li>
                  <li>• Multi-Location Strategy</li>
                  <li>• Conversion Optimization</li>
                </ul>
              </div>
            </div>

            <div className="space-y-8">
              <h3 className="headline-lg text-navy">Success Stories</h3>
              {[
                {
                  business: "Surrey Multi-Trade Company",
                  challenge: "Scattered marketing efforts",
                  result: "Unified strategy = 400% ROI increase",
                  time: "3 months"
                },
                {
                  business: "Epsom Professional Services",
                  challenge: "Low-quality leads",
                  result: "85% improvement in lead quality",
                  time: "2 months"
                },
                {
                  business: "Redhill Manufacturing",
                  challenge: "No digital presence",
                  result: "£500k in new B2B contracts",
                  time: "6 months"
                }
              ].map((study, index) => (
                <div key={index} className="brutal-card">
                  <h4 className="font-bold text-lg mb-2">{study.business}</h4>
                  <p className="mb-2"><strong>Challenge:</strong> {study.challenge}</p>
                  <p className="text-orange font-bold">{study.result}</p>
                  <span className="data-mono text-sm">Delivered in {study.time}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="lg:col-span-1">
            <div className="brutal-card bg-orange text-white sticky top-8">
              <h3 className="headline-lg mb-6">Investment</h3>
              <div className="data-mono text-4xl font-bold mb-4">£1,200/month</div>
              <p className="mb-6">Expert consultation and strategy implementation</p>
              <Button className="brutal-button w-full bg-white text-orange border-white">
                Book Free Consultation
              </Button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default SEOConsultant;